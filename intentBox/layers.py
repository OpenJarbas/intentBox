...
class IntentLayers:
    def __init__(self, bus, layers=None):
        self.bus = bus
        # make intent levels for N layers
        self.layers = []
        self.replace_layers(layers)
        self.current_layer = 0
        self.activate_layer(0)
        self.named_layers = {}

    def replace_layers(self, layers=None):
        self.disable()
        layers = layers or []
    ...
    def activate_layer(self, layer_num):
        ...
        # if an intent is in both layers it may end up in an unexpected state
        sleep(0.3)
        ...
        if layer_num < len(self.layers) and len(self.layers):
            for intent_name in self.layers[layer_num]:
                self.enable_intent(intent_name.replace(" ", "_") + "Intent")
            return True
        return False
    ...
    def deactivate_layer(self, layer_num):
        ...
        for intent_name in self.layers[layer_num]:
            self.disable_intent(intent_name.replace(" ", "_") + "Intent")
        return True
